...
export class MainMenu extends Entity {
  ...
  draw(ctx) {
    ...
    // **Help Button**
    const helpButton = ASSET_MANAGER.getAsset("./assets/ui/menu/buttonHelp.png");
    if (helpButton) {
        if (this.selectedOption === 2) {
            ctx.strokeStyle = "yellow";
            ctx.lineWidth = 5;
            ctx.strokeRect(helpX - 5, helpY - 5, buttonWidth + 10, buttonHeight + 10);
        }
  ...
  executeSelectedOption() {
    ...
    } else if (selectedOption === 2) { // Help option
        this.showHelp = true;
    }
}
