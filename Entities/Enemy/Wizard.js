...
export class Wizard extends Actor {
    constructor() {
        ...
        this.locations = [{ x: 2000, y: 1900 }, { x: 1600, y: 2200 }, { x: 1200, y: 1700 }, { x: 900, y: 2200 }];
    ...
    update() {
        ...
        if (this.timeSinceTeleport > this.maxTeleportTime) {
            ...
            if (this.effects.burn > 0 || this.effects.shock > 0 || this.effects.frozen > 0 || this.effects.void > 0) {
                this.teleport();
            }
                ...
                for (let e of GAME_ENGINE.entities) {
                    if ((e instanceof Fireball || e instanceof Icicle) && Util.getDistance(this, e) < 300) {
                        this.teleport();
                        break;
                    }
                }
        ...
        if (this.timeSinceAttack >= this.maxAttackDelay) {
            ...
            window.ASSET_MANAGER.playAsset("./assets/sfx/wizard_charge.wav", 2 * Util.DFCVM(this));

        } else if (this.animation === 'attack' && this.currentFrame >= 8) {
            this.animation = 'idle';
            this.currentFrame = 0;
        } else if (this.animation === 'attack' && this.currentFrame >= 5 && !this.attackReleased) {
            ...
            if (this.spell === 0) {
                ...
                GAME_ENGINE.addEntity(new EvilIcicle(this, angle + .2));
                GAME_ENGINE.addEntity(new EvilIcicle(this, angle - .2));
    ...
    teleport() {
        ...
        GAME_ENGINE.addEntity(new TeleportLightning(this.locations[this.locationIndex], this.locations[index]));
...
class TeleportLightning extends Entity {
    constructor(start, end) {
        ...
        this.time = 0;
        this.entityOrder = 1;
        window.ASSET_MANAGER.playAsset("./assets/sfx/lightning.wav", .4);
    }
    update() {
    ...
    draw(ctx) {
        ...
        let maxOffset = 120;
        ctx.beginPath();
        ctx.moveTo(this.start.x - GAME_ENGINE.camera.x + Math.random() * maxOffset - maxOffset / 2,
            this.start.y - GAME_ENGINE.camera.y + Math.random() * maxOffset - maxOffset / 2
        );
        ctx.lineTo(this.end.x - GAME_ENGINE.camera.x + Math.random() * maxOffset - maxOffset / 2,
            this.end.y - GAME_ENGINE.camera.y + Math.random() * maxOffset - maxOffset / 2
        );
