...
export class VoidExplosion extends Entity {
  constructor(pos) {
    ...
    this.entityOrder = -0.5;
    this.collider = new Collider(450, 450);
    for (let e of GAME_ENGINE.entities) {
      if (e.isEnemy && e.colliding(this)) {
        e.queueAttack({ damage: 14, void: 4 });
      }
    }
    //this.collider = null;
    window.ASSET_MANAGER.playAsset(
      "./assets/sfx/void_explosion.wav",
      1 * Util.DFCVM(this)
    );
  }
  ...
  draw(ctx) {
    ...
    ctx.drawImage(
      ...
      Math.floor((this.timer / this.expiration_time) * 20) *
